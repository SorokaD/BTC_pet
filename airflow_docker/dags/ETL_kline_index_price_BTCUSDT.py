...
# DAG definition
dag = DAG(f'ETL_kline_index_price_{symbol}',
...
def extract_transform(ti):
    ...
    for i in list(category.keys()):
        for j in list(interval.keys()):
            ...
            query = text("SELECT max(datetime) mdt FROM data.kline_index_price where category=:category and symbol=:symbol and interval=:interval")
            ...
            while current <= end:
                lag = current + equal_dict.get(j)*(limit)
                data = src_API.get_index_price_kline(client, i, symbol, j, start=current, limit=limit)
...
def load(ti):
    ...
    data.to_sql(name='kline_index_price', schema='data', con=engine, if_exists='append', index=False)
    return True
