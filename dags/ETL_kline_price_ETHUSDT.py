...
# DAG definition
dag = DAG(f'ETL_kline_price_{symbol}',
...
def extract_transform(ti):
    ...
    for i in list(category.keys()):
        for j in list(interval.keys()):
            ...
            query = text("SELECT max(datetime) mdt FROM data.kline_price where category=:category and symbol=:symbol and interval=:interval")
...
def load(ti):
    ...
    data.to_sql(name='kline_price', schema='data', con=engine, if_exists='append', index=False)
    return True
